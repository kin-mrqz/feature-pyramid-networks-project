...
// Raw pointer API: useful for modern PyTorch C++ extensions.
// These functions avoid any dependency on TH/TH.h and operate on
// plain C pointers and integer sizes so the wrapper can call them
// from C++/ATen.
...
void crop_and_resize_forward_raw(
    ...
    const int channel_elements = crop_height * crop_width;
    const int crop_elements = depth * channel_elements;
    const int total = num_boxes * crop_elements;
    for (int i = 0; i < total; ++i) crops_data[i] = 0.0f;
...
void crop_and_resize_backward_raw(
    ...
    const int channel_elements = crop_height * crop_width;
    const int crop_elements = depth * channel_elements;
    const int total_image = batch_size * image_elements;
    for (int i = 0; i < total_image; ++i) grads_image_data[i] = 0.0f;
    ...
    for (int b = 0; b < num_boxes; ++b) {
        ...
        const float height_scale =
            (crop_height > 1) ? (y2 - y1) * (image_height - 1) / (crop_height - 1)
                              : 0;
        const float width_scale =
            (crop_width > 1) ? (x3 - x1) * (image_width - 1) / (crop_width - 1)
                             : 0;
